...
| 日期 | 类型 | 变更 | 摘要 |
|------|------|------|------|
| 2026-06-04 | fix | [cwd-validation-and-spawn-cwd-error](2026-06-04-cwd-validation-and-spawn-cwd-error.zh.md) | `/cd` 和 `/new <path>` 落盘前先 stat 校验目标（不存在/非目录直接拒绝，不再写坏 session 拖垮聊天）；`spawnWithLifecycle` 在 cwd 不存在时报 `cwd does not exist: …`，替代误导性的 `spawn <cmd> ENOENT`。`resolveCwd` 去重收敛到 `src/commands/cwd.ts`。 |
