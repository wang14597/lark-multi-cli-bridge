...
| 日期 | 类型 | 变更 | 摘要 |
|------|------|------|------|
| 2026-06-13 | feat | [card-rendering-improvements](2026-06-13-card-rendering-improvements.zh.md) | 运行卡片通过 `config.width_mode: 'fill'` 改为全宽；超过 10 行的回答文本块包裹进默认展开的 `collapsible_panel`（正常字号），用户可原生折叠；短回答与命令卡片不受影响。 |
