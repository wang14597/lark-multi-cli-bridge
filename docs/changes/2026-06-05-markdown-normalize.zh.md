...
- 围栏代码块（```` ``` ````/`~~~`）**原样透传**——内部空行与缩进不动——只在围栏
  外侧补空行。
- 表格行（含 `|` 的行）视为紧凑块，表体绝不被拆开；表格前后补空行。
- 连续空行折叠为单个空行；首尾空行裁掉。
...
- `tests/lark/markdown-normalize.test.ts` —— **新增。** 13 个单测（正文拆分、紧凑
  列表前补空行/列表后补空行、有序列表、标题、引用、围栏代码原样、表格保护、空
  行折叠、裁剪、幂等、空串/单行）。
