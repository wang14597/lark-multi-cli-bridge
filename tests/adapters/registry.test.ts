...
describe('buildAdapter — effective system prompt resolution', () => {
  it('defaults to injecting BOT_SKILL_PROMPT when injectSkillPrompt is absent', () => {
    ...
    expect(adapterOpts(adapter).appendSystemPrompt).toBe(BOT_SKILL_PROMPT);
  });

  it('omits the skill prompt when injectSkillPrompt is false', () => {
    const bot: BotConfig = {
      ...baseClaudeBot,
      backend: { ...baseClaudeBot.backend, injectSkillPrompt: false },
    };
    const adapter = buildAdapter(bot);
    expect(adapterOpts(adapter).appendSystemPrompt ?? '').toBe('');
  });

  it('concatenates appendSystemPrompt after the skill prompt when both set', () => {
    const bot: BotConfig = {
      ...baseClaudeBot,
      backend: { ...baseClaudeBot.backend, appendSystemPrompt: 'EXTRA' },
    };
    const adapter = buildAdapter(bot);
    expect(adapterOpts(adapter).appendSystemPrompt).toBe(BOT_SKILL_PROMPT + '\n\n' + 'EXTRA');
  });
  ...
  it('uses only appendSystemPrompt when skill-prompt is disabled', () => {
    const bot: BotConfig = {
      ...baseClaudeBot,
      backend: { ...baseClaudeBot.backend, injectSkillPrompt: false, appendSystemPrompt: 'EXTRA' },
    };
    const adapter = buildAdapter(bot);
    expect(adapterOpts(adapter).appendSystemPrompt).toBe('EXTRA');
  });
  ...
  it('passes skip_git_repo_check from codex sub-block into adapter opts', () => {
    const codexBot: BotConfig = {
      ...baseClaudeBot,
      name: 'codex-bot',
      backend: { type: 'codex', codex: { json_mode: true, extra_args: [], skip_git_repo_check: false } },
    };
    const adapter = buildAdapter(codexBot);
    expect(adapterOpts(adapter).skipGitRepoCheck).toBe(false);
  });
